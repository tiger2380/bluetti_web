...
class Swidly {
    ...
    public function loadControllerRoutes(): void
    {
        ...
        $controllerRootPath = APP_PATH.DIRECTORY_SEPARATOR.'themes'.DIRECTORY_SEPARATOR. $theme;
        $controllerPath = $controllerRootPath.DIRECTORY_SEPARATOR.'controllers'.DIRECTORY_SEPARATOR;
        ...
        foreach ($controllerFilenames as $controller) {
            $class = new ReflectionClass('Swidly\\themes\\' . $theme . '\\controllers\\' . $controller);
     ...
     * @throws SwidlyException
     */
    static function load_script_file(string $name, string $dir = '/js', $isModule = false): void {
        if(filter_var($name, FILTER_VALIDATE_URL)) {
            echo '<script type="text/javascript" src="'. $name .'"></script>';
        } else {
            $themePath = self::theme();
            $jsDir = $themePath['base'].$dir;
            ...
            if(file_exists($jsDir)) {
                ...
                if(!empty($jsFile)) {
                    ...
                    $_url = $url.( isset($dir) ? $dir.'/' : '' ).$parsed_file['basename'].'?t='. time();
                    echo '<script type="'.$type.'" src="'.$_url.'"></script>';
                    return;
                }
            }
        }
     ...
     * @throws SwidlyException
     */
    static function load_stylesheet_file(string $name, string $dir = '/css'):void {
        if(filter_var($name, FILTER_VALIDATE_URL)) {
            echo '<link rel="stylesheet" href="'. $name .'">';
            return;
        } else {
            $themePath = self::theme();
            $cssDir = $themePath['base'].$dir;
            ...
            if(file_exists($cssDir)) {
                ...
                if(!empty($cssFile)) {
                    echo '<link rel="stylesheet" href="'. $url.( isset($dir) ? $dir.'/' : '' ).$parsed_file['basename'].'?t='. time() .'">';
                }
                return;
            }
        }
    ...
    /**
     * Generate a URL with the given path and parameters.
     *
     * @param string $path The path for the URL
     * @param array $params (Optional) The parameters to be included in the URL
     * @return string The generated URL
     */
    public static function url(string $path, array $params = []): string
    {
        return Swidly::path($path, $params);
    }
}